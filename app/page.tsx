...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '60px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1', minWidth: '300px' }}>
            ...
            <h1 style={{ fontSize: 'clamp(32px, 5vw, 56px)', fontWeight: 800, lineHeight: 1.15, marginBottom: '20px', color: 'var(--text)' }}>
              {t(lang, 'hero_title_1')}<br />
              <span style={{ color: '#00C896' }}>{t(lang, 'hero_title_2')}</span>
            </h1>

            <p style={{ fontSize: '18px', color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: '36px', maxWidth: '480px', whiteSpace: 'pre-line' }}>
      ...
      {/* Stats */}
      <section style={{ background: 'var(--bg-secondary)', padding: '40px 24px', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '32px' }}>
          {stats.map((s) => (
            <div key={s.label} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--bg)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '56px' }}>
            <h2 style={{ fontSize: 'clamp(24px, 4vw, 40px)', fontWeight: 800, color: 'var(--text)', marginBottom: '16px' }}>
              {t(lang, 'features_title')}
            </h2>
            <p style={{ color: 'var(--muted)', fontSize: '18px' }}>{t(lang, 'features_subtitle')}</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            {features.map((f) => (
              <div key={f.title} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '20px', padding: '32px' }}>
                <h3 style={{ fontSize: '22px', fontWeight: 700, color: 'var(--text)', marginBottom: '12px' }}>{f.title}</h3>
                <p style={{ color: 'var(--muted)', lineHeight: 1.7, marginBottom: '24px' }}>{f.desc}</p>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--bg-secondary)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(24px, 4vw, 40px)', fontWeight: 800, color: 'var(--text)', marginBottom: '12px' }}>
              {t(lang, 'categories_title')}
            </h2>
            <p style={{ color: 'var(--muted)', fontSize: '16px' }}>{t(lang, 'categories_subtitle')}</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '16px' }}>
            {categories.map((cat) => (
              <Link key={cat.id} href={`/laws?category=${cat.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px 16px', textAlign: 'center' }}>
      ...
      {/* Download CTA */}
      <section style={{ background: 'linear-gradient(135deg, var(--bg-secondary) 0%, var(--bg) 100%)', padding: '80px 24px', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
          <div style={{ fontSize: '64px', marginBottom: '20px' }}>📱</div>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, color: 'var(--text)', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '20px', textAlign: 'left' }}>
            {downloadSteps.map((step, i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', background: 'var(--bg-secondary)', borderRadius: '12px', padding: '16px', border: '1px solid var(--border)' }}>
                ...
                <span style={{ color: 'var(--text)', fontSize: '14px', lineHeight: 1.5, paddingTop: '3px' }}>{step}</span>
              </div>
            ))}
          </div>
        </div>
