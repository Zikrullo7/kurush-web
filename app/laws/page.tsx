      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '24px' }}>
          ...
          <input
            ...
            style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px 16px 14px 48px', color: 'var(--text)', fontSize: '16px', outline: 'none' }}
          />
          {query && (
            <button onClick={() => setQuery('')} style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: '18px' }}>
              {t(lang, 'laws_clear')}
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '40px' }}>
          <button
            ...
            style={{ padding: '8px 18px', borderRadius: '100px', border: `1px solid ${activeCategory === 'all' ? '#00C896' : 'var(--border)'}`, background: activeCategory === 'all' ? 'rgba(0,200,150,0.15)' : 'transparent', color: activeCategory === 'all' ? '#00C896' : 'var(--muted)', cursor: 'pointer', fontSize: '14px', fontWeight: 600, transition: 'all 0.2s' }}
          >
          ...
          {categories.map((cat) => (
            <button
              ...
              style={{ padding: '8px 18px', borderRadius: '100px', border: `1px solid ${activeCategory === cat.id ? cat.color : 'var(--border)'}`, background: activeCategory === cat.id ? `rgba(${hexToRgb(cat.color)}, 0.15)` : 'transparent', color: activeCategory === cat.id ? cat.color : 'var(--muted)', cursor: 'pointer', fontSize: '14px', fontWeight: activeCategory === cat.id ? 600 : 400, transition: 'all 0.2s' }}
            >
          ...
          <div style={{ marginBottom: '48px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {categories.map((cat) => {
                ...
                return (
                  <button key={cat.id} onClick={() => setActiveCategory(cat.id)} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', textAlign: 'left', cursor: 'pointer', transition: 'all 0.2s' }}
                  ...
                  >
                    ...
                    <div style={{ color: 'var(--text)', fontWeight: 600, fontSize: '16px', marginBottom: '4px' }}>
                      {t(lang, CAT_LABEL_KEYS[cat.id as keyof typeof CAT_LABEL_KEYS] ?? 'cat_traffic')}
                    </div>
                    <div style={{ color: 'var(--muted)', fontSize: '13px' }}>{count} {t(lang, 'laws_articles')}</div>
                  </button>
                )
              })}
            </div>
              ...
              {displayedLaws.map((law) => {
                ...
                return (
                  <Link key={law.id} href={`/laws/${law.id}`} style={{ textDecoration: 'none' }}>
                    <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', height: '100%', transition: 'all 0.2s' }}
                    ...
                    >
                      ...
                      <h3 style={{ color: 'var(--text)', fontWeight: 700, fontSize: '16px', marginBottom: '8px', lineHeight: 1.4 }}>{law.title}</h3>
                      <p style={{ color: 'var(--muted)', fontSize: '13px', lineHeight: 1.6, marginBottom: '16px' }}>{law.description}</p>
